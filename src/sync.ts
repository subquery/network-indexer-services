...
export async function sync(app: INestApplication) {
    ...
    while(true) {
        ...
        for (var i = 0; i <= steps; i++) {
            ...
            _opens.forEach(async (_open) => {
                await paygServicee.sync_open(_open.args.channelId.toString(), _open.args.indexer, _open.args.consumer, _open.args.total.toString(), _open.args.price.toString(), _open.args.expiredAt.toNumber(), _open.args.deploymentId);
            });
    ...
    contract.on("ChannelOpen", async (channelId, indexer, consumer, total, price, expiredAt, deploymentId) => {
        let chain_last_block = await contractService.getLastBlockNumber();
